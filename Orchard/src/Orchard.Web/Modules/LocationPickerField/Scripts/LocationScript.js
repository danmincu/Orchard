(function () {
    ...
    var initialize = function (i, el) {
        ...
        if ($input.hasClass('locationFieldPicker')) {
            $box = '400px'
            $initialZoom = 14;
            $maxZoom = 15;
            $draggable = true;
        ...
        if (!latLong || !latLong.latitude || !latLong.longitude) {
            $initialZoom = 3;
            this.value = "38.2726893782715,-97.6992219686508";
            latLong = {
                latitude: 38.2726893782715,
                longitude: -97.6992219686508
            };
        }
        else {
            needsPosition = false;
        ...
        // Create a "Google(r)(tm)" LatLong object representing our location's lat/long.
        ...
        //// Place a marker on it, representing the location object's position.
        ...
        if ($input.hasClass('locationFieldPicker') && needsPosition) {
            var geoMarker = new GeolocationMarker();
            geoMarker.setCircleOptions({ visible:false, fillColor: '#ffffff' });            
        ...
        var updateMarker = function (updateEvent) {
            ...
            $input.val(marker.getPosition().toUrlValue(13));
            //el.val(marker.getPosition().toUrlValue(13));
        };
    ...
    // Find all location inputs and initialize maps for them.
    $('.locationFieldPicker, .locationFieldPickerDisplay').each(initialize);
})();
